...
var navigation = function navigation() {
  ...
  function scrollTo(selector) {
    document.querySelector("".concat(selector)).scrollIntoView({
      behavior: 'smooth'
    });
    menuWrapper.classList.remove('active');
  }

  var triangle = document.querySelector('.triangle > polyline'),
  ...
  var arrayInstrument = [];
  menuWrapperInstrument.forEach(function (item, i) {
    arrayInstrument[i] = getCoords(item);
  });
  window.addEventListener('scroll', function () {
    beautiful(boxTriangle, triangle, 500);
    arrayInstrument.forEach(function (item, i) {
  ...
  menuWrapperLinks.addEventListener('click', function (event) {
    event.preventDefault();
    var targetid = event.target.href;
